...
console.log(doesItContain("crust", pizza));
console.log(doesItContain("sauce", pizza));
...
function introduceTheActors(actorArray){
  ...
  for(let i = 0; i < actorArray.length; i++) {
    newActorArray.push(`Hi, I'm ${actorArray[i].name} and I was in ${actorArray[i].movie}.`);
  }
  return newActorArray;
}
let newActors = introduceTheActors(actors);
...
function storeFounders(obj){
  newArray = [];
  for (element in obj){
    if(typeof(obj[element]) === 'string') newArray.push(obj[element]);
  }
  return newArray;
};
